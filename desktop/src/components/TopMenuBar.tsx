...
export default function TopMenuBar({ isDark, onToggleTheme }: Props) {
  ...
  const handleNewChat = () => {
    if (location.pathname !== "/") {
      navigate("/");
      // Let the home page load then trigger new chat
      setTimeout(() => {
  ...
  const handleExit = async () => {
    try {
      // In Tauri v2 we can use the window api to close
  ...
  const helpItems: MenuProps["items"] = [
    {
      ...
      onClick: () => window.open("https://github.com/zhengbohou0402/FTSM-RAG", "_blank"),
    },
    {
      type: "divider",
    },
      ...
      >
        <Space direction="vertical" size="middle" style={{ width: "100%", textAlign: "center", padding: "10px 0" }}>
          ...
          <Text style={{ display: "block" }}>
            A FastAPI + React + Qdrant based RAG assistant designed for UKM FTSM student onboarding guidelines and academic calendars.
          </Text>
          ...
          <Text type="secondary" style={{ fontSize: "12px" }}>
            Project Repo: <AntLink href="https://github.com/zhengbohou0402/FTSM-RAG" target="_blank">zhengbohou0402/FTSM-RAG</AntLink>
          </Text>
        </Space>
      </Modal>
    </div>
